...
class Cnn:
    ...
    def __init__(self, data_sample=None):
        ...
        self.graph = self.build_graph(top_k=3, images=data_sample[0], labels=data_sample[1])
    ...
    def training(self):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=self.gpu_options, allow_soft_placement=True,
                                              log_device_placement=True)) as sess:

            training_init_op = training_data.get_batch(batch_size=self.FLAGS.batch_size, aug=True)
            next_training_sample = training_data.get_next_element()

            graph_input = next_training_sample[0]
            tf.identity(graph_input, name='input')

            graph = Cnn.build_graph(top_k=1, images=graph_input, labels=next_training_sample[1])
            ...
            eval_frequency = self.FLAGS.evaluation_step_frequency
            # starts training
            while not coordinator.should_stop():

                step = train()
                if step > self.FLAGS.max_steps:
                    break
                ...
                if ((step % self.FLAGS.saving_step_frequency == 0) and (step != 0)):
                    logger.info('Saving checkpoint of step %s' % step)
                    saver.save(sess, os.path.join(self.FLAGS.checkpoint_dir, 'online_hanzi_recog'),
            ...
            train_writer.close()
            test_writer.close()
            saver.save(sess, os.path.join(self.FLAGS.checkpoint_dir, 'online_hanzi_recog'), global_step=graph['step'])
